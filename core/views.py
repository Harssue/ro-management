...
from .models import Customer, AddSpareParts
...
class AddService(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            existing_part = AddSpareParts.objects.get(id=parts_used.id)
...
class AddSparePart(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                existing_part = AddSpareParts.objects.get(serial_number=serial_number)
                existing_part.opening_qty += opening_qty
                existing_part.save()
            except AddSpareParts.DoesNotExist:
                form.save()
            return redirect('/spare-master')
        return render(request, 'add-spare.html', {'form':form})

def spare_master(request):
    spare = AddSpareParts.objects.all()
