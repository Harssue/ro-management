...
class AddSpareParts(models.Model):
...
class ServiceHis(models.Model):
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='service_history')
    service_date = models.DateField()
    parts_used = models.ForeignKey(AddSpareParts, on_delete=models.CASCADE, related_name='parts_used')
    opening_qty = models.IntegerField()

    def __str__(self):
        return f'{self.customer.name} - {self.service_date}'

class PurchaseFormModel(models.Model):
